...
export async function sendMessageToGroq(
  ...
  if (!GROQ_API_KEY || GROQ_API_KEY === '') {
    throw new Error(
      'Chave API do Groq não configurada. Configure VITE_GROQ_API_KEY no arquivo .env'
    );
  }
  ...
  if (!isSearchConfigured()) {
    throw new Error(
      'API de busca não configurada. Configure VITE_TAVILY_API_KEY no arquivo .env para habilitar busca na web obrigatória.'
    );
  }
  ...
  try {
    ...
    if (searchError instanceof Error) {
      if (searchError.message.includes('API')) {
        ...
        errorMessage = 'Chave da API de busca inválida. Verifique VITE_TAVILY_API_KEY no .env';
